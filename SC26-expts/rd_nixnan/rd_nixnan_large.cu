// rd_nixnan_large.cu
// -----------------------------------------------------------------------------
// Larger-domain variant of rd_nixnan.cu: 10x the grid points (N), with L
// scaled up by the same factor so dx (and therefore r, lambda*dt, and the
// blow-up step count) is unchanged -- only the amount of per-step grid data
// crossing each format's overflow edge grows.
//
...
#ifndef N
#define N 1001         // grid points (10x rd_nixnan.cu); dx = L/(N-1) = 0.01
#endif
#ifndef M
...
static const double L      = 10.0;    // 10x rd_nixnan.cu, keeps dx (and r, lambda*dt) unchanged
