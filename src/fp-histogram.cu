...
static std::map<std::string, BinArray*> device_bins;
...
BinArray* make_bin_array(const std::string& function, const json& bin_spec_json,
    ...
    cudaMemcpy(device_bins[function], host_bins, NUM_FORMATS * sizeof(BinArray), cudaMemcpyHostToDevice);
...
BinArray* process_bin_spec(const std::string& function) {
    json bin_spec_json;
    if (bin_spec_file == "") {
...
if (histogram_enabled) {
    ...
    recv_thread = std::thread(recv_thread_fun<exp_info>,
                              ...
                              channel_host,
                              [](exp_info* data) {
                                  static std::map<int, size_t> report_counts;
...
if (histogram_enabled) {
    ...
    {
        BinArray host_bins[NUM_FORMATS];
        for (auto [function, device_bins_ptr] : device_bins) {
            cudaMemcpy(host_bins, device_bins_ptr, NUM_FORMATS * sizeof(BinArray), cudaMemcpyDeviceToHost);
            for (auto fmt : {BF16, FP16, FP32, FP64}) {
                BinCounter* d_cnts = host_bins[fmt].bins;
                cudaFree(d_cnts);
            }
            cudaFree(device_bins_ptr);
        }
    }
}
}
