...
std::string locTupleToLoc(LocationTuple &loc) {
  ...
  // std::cout << "1 = " << std::get<1>(loc) << ", 0 = " << std::get<0>(loc) <<
  // ", 2 = " << std::get<2>(loc) << std::endl;
...
bool is_MMA_INSTRUCTION(std::string inst, uint32_t &reg_type) {
  if (inst.find("HMMA.16816.F16") == 0) {
    ...
    return true;
             ...
             inst.find("HMMA.1688.F32.TF32") == 0) {
    reg_type = FP32;
    return true;
...
void print_exc(std::string loc, std::string f_type, std::string exc,
               int opcode_id, int kernel_id, int f_type_id, uint32_t loc_id,
               int exc_id) {
  ...
  if (it == locExc_count.end()) {
    ...
    if (exc_id == E_SUB) {
      std::cout << "#GPU-FPX LOC-EXCEP INFO: Warning: in kernel ["
                << id_kernel_map[kernel_id] << "], " << "very small quantity ("
                << exc << ") found @ " << loc << " [" << f_type << "]"
                << std::endl;
    } else {
      std::cout << "#GPU-FPX LOC-EXCEP INFO: in kernel ["
                << id_kernel_map[kernel_id] << "], " << exc << " found @ "
                << loc << " [" << f_type << "]" << std::endl;
    }
    if (print_ill_instr) {
      std::cout << "#GPU-FPX VERBOSE:the ill instruction is: "
                << id_to_sass_map[opcode_id] << std::endl;
    }
  } else {
    it->second++;
  }
...
int computeExcNum(int exc_id, int f_type_id) {
  int num = 0;
  std::string pre =
      std::to_string(f_type_id) + "_" + std::to_string(exc_id) + "_";
...
void print_ana_exceps(std::string loc, int opcode_id, int kernel_id,
                      uint32_t loc_id, uint32_t ei_list[], int ops,
                      uint32_t w_lit_except, uint32_t after_before) {
  ...
  std::string ind = std::to_string(loc_id) + "_" + std::to_string(opcode_id) +
                    "_" + std::to_string(after_before);
  for (int i = 0; i < 4; i++) {
  ...
  if (it == locExc_count.end()) {
    ...
    if (after_before) {
      if (after_before == 1) {
        std::cout << "#GPU-FPX-ANA SHARED REGISTER: Before executing the "
                     "instruction ";
        std::cout << " @ " << loc << " ";
        std::cout << "Instruction: " << sass_instr;
        std::cout << " We have " << ops << " registers in total. ";
        for (int i = 0; i < ops; i++) {
          std::cout << "Register " << i << " is "
                    << exceptionAnaTypeNames[ei_list[i]] << ". ";
        }
        std::cout << "\n\n";
      ...
      if (enable_compare) {
        std::cout << "#GPU-FPX-ANA COMPARE: Exceptional value may appear in a "
                     "comparision or min/max statement";
        std::cout << " @ " << loc << " ";
        std::cout << "Instruction: " << sass_instr;
        std::cout << " We have " << ops << " registers in total. ";
        for (int i = 0; i < ops; i++) {
      ...
      if (dest_src_same_e && dest_except) {
        if (progagated) {
          std::cout << "#GPU-FPX-ANA PROGAGATION: Exceptional values are "
                       "progagated from source to destination ";
          std::cout << " @ " << loc << " ";
          std::cout << "Instruction: " << sass_instr;
          std::cout << " We have " << ops << " registers in total. ";
          for (int i = 0; i < ops; i++) {
        ...
        std::cout << "#GPU-FPX-ANA APPEAR : "
                  << exceptionAnaTypeNames[ei_list[0]]
        ...
        std::cout
            << "#GPU-FPX-ANA DISAPPEAR : Exceptional values are disappeared ";
        std::cout << " @ " << loc << " ";
        std::cout << "Instruction: " << sass_instr;
        std::cout << " We have " << ops << " registers in total. ";
        for (int i = 0; i < ops; i++) {
