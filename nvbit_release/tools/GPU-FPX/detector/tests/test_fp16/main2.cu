...
int main(void) {
  ...
  gpuErrchk(
      cudaMemcpy(d_in, h_in, n * sizeof(__half2), cudaMemcpyHostToDevice));
  ...
  gpuErrchk(
      cudaMemcpy(h_in, d_in, n * sizeof(__half2), cudaMemcpyDeviceToHost));
  for (int i = 0; i < n; i++) {
    if ((__high2float(h_in[i]) != 2.5) || (__low2float(h_in[i]) != 2.5)) {
      cout << "Mismatch at " << i << " Expected = 2.5 "
           << "Actual = " << __half2float(h_in[i].x) << " "
           << __half2float(h_in[i].y) << endl;
      exit(1);
    }
    cout << "Actual = " << __half2float(h_in[i].x) << " "
         << __half2float(h_in[i].y) << endl;
  }
