...
__global__ void half2_plus1(int n, __half *in_array) {
  ...
  if (idx % 2 == 0 && idx < n) {
    ...
    // in_array2[idx/2] = __hadd2(__halves2half2(in_array[idx],
    // in_array[idx+1]),  __halves2half2(__float2half(1.9),__float2half(7.4)));
    in_array2[idx / 2] = __hadd2(
        __halves2half2(in_array[idx], in_array[idx + 1]), __float2half2_rn(1));
  }
}
int main(void) {
  const int n = 64;
  ...
  // gpuErrchk( cudaMemcpy(d_in, h_in, n*sizeof(__half2),
  // cudaMemcpyHostToDevice) ); dim3 block_dims(2,1,1); dim3
  // thread_dims(32,1,1); half2_plus1<<<block_dims, thread_dims>>>(d_in);
  // gpuErrchk( cudaPeekAtLastError() );
  // gpuErrchk( cudaDeviceSynchronize() );
  // gpuErrchk( cudaMemcpy(h_in, d_in, n*sizeof(__half2),
  ...
  for (int i = 0; i < n; i++) {
    ...
    // {
    //     cout<< "Mismatch at " << i << " Expected = 2.5 " << "Actual = " <<
    //     __half2float(h_in[i].x) << " " << __half2float(h_in[i].y) << endl;
    //     exit(1);
    // }
