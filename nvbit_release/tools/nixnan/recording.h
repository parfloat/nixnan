...
namespace nixnan {
    class device_recorder {
        public:
            ...
            __device__ inline
            uint32_t record(uint32_t id, uint32_t exce) const {
                size_t index = id << 4 | exce;
    ...
    public:
        ...
        recorder(size_t sz);
        ~recorder();
        uint32_t mk_entry(Instr *instr, uint32_t type, CUcontext ctx, CUfunction f);
        uint32_t mk_entry(std::string& instr, std::string& path, std::string& line, std::string& func, uint32_t type);
        void free_device();
        void end();
        ...
        uint32_t get_type(uint32_t id);
        uint32_t get_exce(uint32_t id);
